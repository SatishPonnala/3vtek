...
window.slugify = function(s){ return s.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,""); };

function p(brand, name, category, subcategory, badges){
  return { id: window.slugify(brand+"-"+name), brand: brand, name: name, category: category, subcategory: subcategory, badges: badges };
}
...
function parseList(s){ if(!s) return undefined; var a=s.split("|").map(function(x){return x.trim();}).filter(Boolean); return a.length?a:undefined; }
function parseSpecs(s){ if(!s) return undefined; var o={}; s.split("|").forEach(function(pair){ var i=pair.indexOf(":"); if(i===-1) return; var k=pair.slice(0,i).trim(); var v=pair.slice(i+1).trim(); if(k) o[k]=v; }); return Object.keys(o).length?o:undefined; }
function parseCsv(text){
  ...
  var idx = function(k){ return header.indexOf(k); };
  var colSlug = idx("slug");
  if(colSlug===-1) return [];
  return rows.slice(1).map(function(line){
    var cells = splitCsvLine(line);
    var get = function(k){ var i=idx(k); return i>=0 ? cells[i] : undefined; };
    var slug = get("slug");
    if(!slug) return {};
    return {
      id: slug, name: get("name")||undefined, brand: get("brand")||undefined,
      category: get("category")||undefined, subcategory: get("subcategory")||undefined,
...
window.loadProducts = function(){
  return fetch("assets/products.csv", { cache: "no-cache" }).then(function(r){
    if(!r.ok) return window.BASE_PRODUCTS.slice();
    return r.text().then(function(text){
      var overrides = parseCsv(text);
      var map = {};
      window.BASE_PRODUCTS.forEach(function(b){ map[b.id]=Object.assign({}, b); });
      overrides.forEach(function(row){
        if(!row.id) return;
        var ex = map[row.id];
        if(ex){
          Object.keys(row).forEach(function(k){ if(row[k] !== undefined) ex[k] = row[k]; });
          map[row.id] = ex;
  ...
  }).catch(function(){ return window.BASE_PRODUCTS.slice(); });
};
...
window.productImage = function(p){ return window.productImages(p)[0]; };
